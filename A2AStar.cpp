...
string solvePuzzle(vector<vector<int>> &last, priority_queue<P, vector<P>, greater<P>> &pq)
{
    string ans = "";
    ...
    cout << "Minimum number of moves to achieve Goal State are: " << ans.length() << endl;
    return ans;
}
int main()
{
    ...
    vector<vector<int>> last(3, vector<int>(3, 0));
    ...
    string s = solvePuzzle(last, pq);
    displayPath(s, grid, findblank(grid).first, findblank(grid).second);
